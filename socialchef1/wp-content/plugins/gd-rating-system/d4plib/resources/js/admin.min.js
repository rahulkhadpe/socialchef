...
var d4plib_admin;(function($,c,d,f){d4plib_admin={scroll_offset:40,active_element:null,init:function(){$(".d4p-nav-button > a").click(function(e){e.preventDefault();$(this).next().slideToggle("fast")});setTimeout(function(){$(".d4p-wrap .updated").slideUp("slow")},4000);$(c).bind("load resize orientationchange",function(){if(d.body.clientWidth<800){d4plib_admin.scroll_offset=60}else{d4plib_admin.scroll_offset=40}if(d.body.clientWidth<640){$(".d4p-panel-scroller").removeClass("d4p-scroll-active")}else{$(".d4p-panel-scroller").addClass("d4p-scroll-active")}});$(".d4p-check-uncheck a").click(function(e){e.preventDefault();var a=$(this).attr("href").substr(1)==="checkall";$(this).parent().parent().find("input[type=checkbox]").prop("checked",a)})},settings:function(){d4plib_media_image.init();$(".d4p-setting-number input, .d4p-field-number").numeric();$(".d4p-setting-integer input, .d4p-field-integer").numeric({decimalPlaces:0,negative:false});$(".d4p-color-picker").wpColorPicker();$(d).on("click",".d4p-group h3 i.fa.fa-caret-down, .d4p-group h3 i.fa.fa-caret-up",function(){var a=$(this).hasClass("fa-caret-down"),content=$(this).parent().next();if(a){$(this).removeClass("fa-caret-down").addClass("fa-caret-up");content.slideDown(300)}else{$(this).removeClass("fa-caret-up").addClass("fa-caret-down");content.slideUp(300)}});$(d).on("click",".d4p-section-toggle .d4p-toggle-title",function(){var a=$(this).find("i.fa"),closed=a.hasClass("fa-caret-down"),content=$(this).next();if(closed){a.removeClass("fa-caret-down").addClass("fa-caret-up");content.slideDown(300)}else{a.removeClass("fa-caret-up").addClass("fa-caret-down");content.slideUp(300)}});$(d).on("click",".d4p-setting-expandable_pairs .button-secondary",function(e){e.preventDefault();$(this).parent().fadeOut(function(){$(this).remove()})});$(d).on("click",".d4p-setting-expandable_text .button-secondary",function(e){e.preventDefault();$(this).parent().fadeOut(function(){$(this).remove()})});$(".d4p-setting-expandable_pairs a.button-primary").click(function(e){e.preventDefault();var b=$(this).closest(".d4p-setting-expandable_pairs"),next=$(".d4p-next-id",b),next_id=next.val(),el=$(".pair-element-0",b).clone();$("input",el).each(function(){var a=$(this).attr("id").replace("_0_","_"+next_id+"_"),name=$(this).attr("name").replace("[0]","["+next_id+"]");$(this).attr("id",a).attr("name",name)});el.attr("class","pair-element-"+next_id).fadeIn();$(this).before(el);next_id++;next.val(next_id)});$(".d4p-setting-expandable_text a.button-primary").click(function(e){e.preventDefault();var b=$(this).closest(".d4p-setting-expandable_text"),next=$(".d4p-next-id",b),next_id=next.val(),el=$(".exp-text-element-0",b).clone();$("input",el).each(function(){var a=$(this).attr("id").replace("_0_","_"+next_id+"_"),name=$(this).attr("name").replace("[0]","["+next_id+"]");$(this).attr("id",a).attr("name",name)});el.attr("class","exp-text-element exp-text-element-"+next_id).fadeIn();$("ol",b).append(el);next_id++;next.val(next_id)})},scroller:function(){var a=$(".d4p-panel-scroller"),$window=$(c);if(a.length>0){var b=a.offset();$window.scroll(function(){if($window.scrollTop()>b.top&&a.hasClass("d4p-scroll-active")){a.stop().animate({marginTop:$window.scrollTop()-b.top+d4plib_admin.scroll_offset})}else{a.stop().animate({marginTop:0})}})}}};d4plib_admin.init();d4plib_admin.settings();d4plib_admin.scroller()})(jQuery,window,document);
